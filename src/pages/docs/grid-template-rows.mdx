...
使用 `grid-rows-{n}` 功能类来创建有_n_个同等大小的行的网格。
...
默认情况下，Tailwind 包括 grid-template-row 功能，用于创建基本的网格，最多有6个等高的行。您可以通过定制您的 Tailwind 主题配置的 `gridTemplateRows` 部分来改变、添加或删除它们。
