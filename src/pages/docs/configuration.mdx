...
使用 Tailwind CLI 功能生成 Tailwind 配置文件，Tailwind CLI 在你安装 `tailwindcss` npm 软件包时已经附带安装过。
...
如果你想在生成 `tailwind.config.js` 文件的同时也生成一个基础的 `postcss.config.js` 文件，请使用 `-p` 标志。
...
对于大多数用户，我们建议您尽量减少配置文件，只指定你想自定义的内容。
...
你将得到一个与 Tailwind 内部使用的 [默认配置文件](https://unpkg.com/browse/tailwindcss@latest/stubs/defaultConfig.stub.js) 一致的文件。
...
在 `theme` 部分中，你可以定义调色板、字体、类型比例、边框大小、断点等任何与你网站视觉设计有关的东西。
...
`variants` 部分允许你控制为每个核心功能插件生成哪些 [变体](/docs/hover-focus-and-other-states) 。
...
`prefix` 选项允许你为所有 Tailwind 生成的功能类添加一个自定义前缀。当 Tailwind 和一个已有的 CSS 存在命名冲突时，这个功能会非常有用。

<!-- For example, you could add a `tw-` prefix by setting the `prefix` option like so: -->
例如，你可以通过这样设置来添加 `tw-` 前缀：
...
如果你也想给你自己的功能类添加前缀，只需要把前缀添加到类定义中即可：
...
`important` 选项允许你控制是否将 Tailwind 的功能类标记为 `!important`。当你将 Tailwind 与已存在的具有非常特殊的选择器的 CSS 一想使用时，这可能会非常有用。

<!-- To generate utilities as `!important`, set the `important` key in your configuration options to `true`: -->
要生成 `!important` 的功能类，在你的配置选项中把 `important` 键设置为 `true`：
...
在合并第三方 JS 库且这些库会为你的元素添加内联样式的时候，设置 `important` 为 `true` 会引入一些问题。在这种情况下，Tailwind 的 `!important` 功能会覆盖内联样式，这会破坏你的预期设计。

<!-- To get around this, you can set `important` to an ID selector like `#app` instead: -->
为了解决这个问题，你可以为一个 ID 选择器设置 `important`，比如 `#app`：
...
这个配置将在你所有的功能类前加上给定的选择器，有效地增加它们的特殊性，而实际上并没有使它们变得 `!important`。
...
当你指定了 `important` 选择器后，你需要确保你的网站的根元素与之匹配。 使用上面的例子，我们需要将根元素的 `id` 属性设置为 `app`，以使样式正常工作。
...
当你的配置都设置好，且你的根元素与 Tailwind 配置中的选择器相匹配后，Tailwind 的所有功能类将有足够高的特异性来击败你的项目中使用的其他类，但**并不**干扰内联样式。
...
`separator` 选项可以让你自定义应该使用什么字符或字符串来分隔变体前缀（屏幕大小、`hover`、`focus`等）和类名（`text-center`、`items-end`等）。
...
我们默认使用冒号(`:`)，但是如果你使用的是像 [Pug](https://pugjs.org) 这样的模板语言，不支持在类名中使用特殊字符，那么改变这个冒号就会很有用。
...
如果您使用 `extend` 功能启用额外的变体，这些变体会自动排序，以尊重合理的默认变体顺序。

<!-- You can customize this if necessary under the `variantOrder` key: -->
如果有必要，你可以在  `variantOrder` 键下面自定义变体顺序：
...
如果你想禁用特定的核心插件，为 `corePlugins` 提供一个对象，将这些插件设置为`false`。
...
如果你想安全地列出哪些核心插件应该被启用，请提供一个数组，其中包括你想使用的核心插件的列表。
...
在你的客户端 JavaScript 中引用配置的值往往非常有用。例如，在 React 或者 Vue 组件中动态应用内联样式的时候需要获取你的主题的配置值。
