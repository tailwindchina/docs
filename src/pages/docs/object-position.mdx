...
要仅在特定的断点处定位对象，请在任何现有的对象定位工具前添加 `{screen}:` 前缀。例如，将 `md:object-top` 类添加到一个元素中，就可以在中等大小和更大的屏幕上应用 `object-top` 功能类。
...
默认情况下，Tailwind 提供了九个对象位置工具。您可以通过编辑您的 Tailwind 配置的 `theme.objectPosition` 部分来改变、添加或删除它们。
