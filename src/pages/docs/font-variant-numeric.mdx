...
使用 `font-variant-numeric` 工具为数字、分数和序号标记启用其他字形（在支持它们的字体中）。
...
这些工具是可组合的，因此你可以通过组合 HTML 中的多个类来实现多个 `font-variant-numeric` 功能。
...
请注意，很多字体不支持这些功能*（例如，对堆叠分数的支持特别少见），因此，根据你所使用的字体序列，其中的一些工具可能无效。

### Ordinal

<!-- Use the `ordinal` utility to enable special glyphs for the ordinal markers. -->
使用 `ordinal` 工具来为序数标记启用特殊字形。
...
使用 `slashed-zero` 功能强制为 0 加上一个短杠。当你需要明确区分 O 和 0 时，这很有用。
...
使用 `lining-nums` 工具来使用所有按其基线对齐的数字字形。这对应于 `lnum` OpenType功能。这是大多数字体的默认值。
...
使用 `oldstyle-nums` 工具来使用其中一些数字有降格的数字字形。这对应于 `onum` OpenType功能。
...
使用 `proportional-nums` 工具来使用具有比例宽度（而不是统一/表格）的数字字形。这对应于 `pnum` OpenType功能。
...
使用 `tabular-nums` 工具来使用具有统一/表格宽度（而不是比例）的数字字形。这与 `tnum` OpenType功能相对应。
...
使用 `diagonal-fractions` 工具，用普通的对角线分数替换被斜线分隔的数字。这对应于 `frac` OpenType功能。
...
使用 `stacked-fractions` 工具，用常见的堆叠分数替换用斜线分隔的数字。这对应于 `frac` OpenType功能。似乎很少有字体支持这个功能--我们在这里使用的是 Ubuntu Mono。
...
要在特定的断点处控制元素的 `font-variant-numeric` 属性，请在任何现有的 `font-variant-numeric` 工具类前添加 `{screen}:` 前缀。例如，使用 `md:tabular-nums` 来仅在中等大小及以上的屏幕应用 `tabular-nums` 工具。
