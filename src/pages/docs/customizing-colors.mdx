...
但是当你需要定制你的调色板时，你可以在你的 `tailwind.config.js` 文件的 `theme` 部分的 `colors` 键下配置你的颜色。
...
当涉及到建立一个自定义调色板时，你可以从我们丰富的调色板中 [策划你的颜色](#curating-colors)，或者通过直接添加你的特定颜色值 [配置你自己的自定义颜色] (#custom-colors)。
...
如果你没有一套完全自定义的颜色，你可以从我们完整的调色板中策划你的颜色，只需要将 `'tailwindcss/colors'` 导入你的配置文件中，然后选择你喜欢的颜色。
...
虽然每种颜色都有一个特定的名称，但我们鼓励你在自己的项目中按照自己的喜好给它们起别名。我们甚至在默认配置中也是这样做的，给 `coolGray` 起别名为 `gray`，给 `violet` 起别名为 `purple`，给 `amber` 起别名为 `yellow`，给 `emerald` 起别名为 `green`。
...
你可以看到，上面我们使用嵌套对象符号来定义我们的颜色，其中嵌套键作为修饰符添加到基础颜色名称中：
...
请注意，当使用 `theme()` 函数访问颜色时，你需要使用与定义颜色相同的符号。
...
正如 [主题文档](/docs/theme#extending-the-default-theme) 中所述，如果你想扩展默认的调色板，而不是覆盖它，你可以使用你的 `tailwind.config.js` 文件中的 `theme.extend.colors` 部分来实现。
...
这些扩展是深度合并的，所以如果你想在 Tailwind 的默认颜色中增加一个额外的阴影，你可以这样做：
...
如果你想禁用一个默认颜色，因为你没有在项目中使用它，最简单的方法是建立一个新的调色板，不包括你想禁用的颜色。
...
另外，你也可以不动调色板，依靠 [tree-shaking 未使用的样式](/docs/optimizing-for-production) 来删除你不使用的颜色。
...
例如，如果你正在做一个需要支持多个主题的项目，那么使用 `primary` 和 `secondary` 这样更抽象的名称可能是有意义的。
...
你可以像我们上面那样明确地配置这些颜色，也可以从我们完整的调色板中调入颜色，并对给他们起个别名。
...
你甚至可以使用 CSS 自定义属性（变量）来定义这些颜色，以便于在客户端切换主题。
...
当你把 `tailwindcss/colors` 导入到你的 `tailwind.config.js` 文件中时，这是所有可用颜色的列表。
...
虽然每种颜色都有一个特定的名称，但我们鼓励你在自己的项目中按照自己的喜好给它们起别名。
