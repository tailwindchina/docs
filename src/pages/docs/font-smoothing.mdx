...
使用 `subpixel-antialiased` 工具以使用 subpixel antialiasing 来渲染文本。
...
要在特定的断点处控制一个元素的字体平滑度，可以在任何现有的字体平滑度工具前添加一个 `{screen}:` 前缀。例如，使用 `md:antialiased` 来仅在中等及以上尺寸屏幕上应用 `antialiased` 工具。
