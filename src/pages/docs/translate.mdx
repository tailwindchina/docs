...
平移一个元素时，首先要使用 `transform` 功能类启用变换，然后使用 `translate-x-{amount}` 和 `translate-y-{amount}` 功能类指定平移方向和距离。
...
要在特定的断点处控制元素的变换，可在任何现有的变换功能类中添加 `{screen}:` 前缀。例如，使用 `md:translate-x-8` 来应用 `translate-x-8` 功能，只在中等大小的屏幕上使用。
...
默认情况下，Tailwind 提供了固定值的平移功能类，它与我们的[ spacing scale ](/docs/customizing-spacing)相匹配，以及相对于元素大小的 50% 和 100% 的平移变化。
...
要单独定制平移比例，请使用您的 `tailwind.config.js` 文件中的 `theme.translate` 部分。
