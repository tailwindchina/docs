...
要开始你的第一个插件，先从 `tailwindcss/plugin` 导入 Tailwind 的 `plugin` 函数。然后在你的 `plugins` 数组中，调用这个函数，用一个匿名函数作为其第一个参数。
...
要从插件中添加新的功能类，调用 `addUtilities`，使用 [CSS-in-JS 语法](#css-in-js-syntax) 传递你的样式。
...
如果有必要，你可以通过向 `addUtilities` 传递一个选项对象作为第二个参数来选择不使用这种行为。
...
要生成 responsive、hover、focus、active 或样式的其它变体，请使用 `variants` 选项指定你想生成的变体。
...
如果你只需要指定变体，而不需要选择不使用默认的前缀或 important 选项，你也可以直接将变体数组作为第二个参数传递。
...
如果你希望用户在他们的 `tailwind.config.js` 文件中的 `variants` 部分提供自己的变体，你可以使用 `variants()` 函数来获取他们配置的变体。
...
用它来添加更个性化、更复杂的类，比如按钮、表单控件、警告等；就是你在其他框架中经常看到的你可能需要使用功能类覆盖的那种预建组件。
...
要从插件中添加新的组件样式，调用 `addComponents` ，使用 [CSS-in-JS 语法](#css-in-js-syntax) 传递你的样式。
...
虽然很少有一个很好的理由让组件声明 important，但如果你真的需要这样做，你可以手动添加 `!important` 。
...
选择器中的所有类都会默认添加前缀，所以如果你添加一个更复杂的样式，比如：
...
要从插件中添加新的基础样式，调用 `addBase`，使用 [CSS-in-JS 语法](#css-in-js-syntax) 传递你的样式。
...
如果你的插件生成的类中包含用户提供的字符串，你可以使用 `e` 函数来转义这些类名，以确保非标准字符被正确地自动处理。
...
注意只转义你真正想转义的内容；不要在类名中传递前导的 `.` 或在 `:hover` 或 `:focus` 等伪类的开头传递 `:`，否则这些字符将被转义。
...
此外，因为 CSS 有关于类名的规则（类名不能以数字开头，但可以包含数字），所以最好是转义完整的类名（不只是用户提供的部分），否则你可能会得到不必要的转义序列。
...
如果你正在写一些复杂的东西，你只想给某些类加前缀，你可以使用 `prefix` 函数来精细控制何时应用用户配置的前缀。
...
例如，如果你正在创建一个插件，以便在一组内部项目中重复使用，在其选择器中包含现有的类，你可能只想给新的类加上前缀。
...
`config`、`theme` 和 `variants` 函数允许你使用点符号从用户的 Tailwind 配置中获取一个值，如果该路径不存在，则提供一个默认值。
...
如果你想引用用户的 `variants` 配置，建议你使用 `variants()` 函数而不是 config 函数。
...
由于 `variants` 可以简单地成为一个全局的变体列表，为整个项目中的每一个插件进行配置，所以使用 `variants()` 函数可以让你很容易地遵从用户的配置，而不是自己重新实现这个逻辑。
...
最好的方法是在用户的 `theme` 和 `variants` 配置中定义自己的键，并要求他们在那里提供任何选项，这样你就可以通过 `theme` 和 `variants` 函数访问它们。
...
要使用它，你需要在你的插件列表中 `require` 它，在 `theme` 和 `variants` 中的 `gradients` 键下指定你的配置。
...
每一个 `addUtilities`、`addComponents` 和 `addBase` 都期望 CSS 规则写成 JavaScript 对象。Tailwind 使用的语法，你可能会从 CSS-in-JS 库中识别出来，如 [Emotion](https://emotion.sh/docs/object-styles)，并由 [postcss-js](https://github.com/postcss/postcss-js) 提供支持。
...
也支持嵌套（由 [postcss-nested](https://github.com/postcss/postcss-nested) 提供支持），使用你可能熟悉的 Sass 或 Less 的相同语法。
...
`addVariant` 函数允许你注册自己的自定义 [变体](/docs/hover-focus-andother-states)，这些变体可以像内置的 hover、hover、hover 等变体一样使用。
...
要添加一个新的变量，调用 `addVariant` 函数，传入你的自定义变量的名称，以及一个回调，根据需要修改受影响的 CSS 规则。
...
如果你想添加一个简单的变量，只需要改变选择器，使用 `modifySelectors` 助手。
...
你传递给 `modifySelectors` 的函数应该简单地返回修改后的选择器。
...
如果你需要做的事情不仅仅是简单地修改选择器（比如改变实际的规则声明，或者将规则包装在另一个 at-rule 中），你将需要使用 `container` 实例。
...
使用 `container` 实例，你可以在给定的模块或 `@variants` 块中遍历所有的规则，并使用标准的 PostCSS API 随意操作它们。
...
要在你自己的CSS中使自定义变体和自定义功能类一起使用，请使用 [variants at-rule](/docs/functions-and-directives#variants)。
