...
要在特定的断点处控制元素的字母间距，请在任何现有的字母间距工具类前添加 `{screen}:` 前缀。例如，使用 `md:tracking-wide` 来仅在中等大小及以上的屏幕应用 `tracking-wide` 工具。
...
默认情况下，Tailwind 提供了六个字距工具。您可以通过编辑您的 Tailwind 配置中的 `theme.letterSpacing` 部分来改变、添加或删除这些工具。
