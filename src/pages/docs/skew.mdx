...
首先使用 `transform` 功能启用 transforms，然后使用 `skew-x-{amount}` 和 `skew-y-{amount}` 功能指定一个元素的倾斜角度。
...
要在特定的断点处控制元素的倾斜度，请在任何现有的倾斜度功能类中添加 `{screen}:` 前缀。例如，使用 `md:skew-6` 来应用 `skew-6` 功能，只适用于中等尺寸以上的屏幕。
...
默认情况下，Tailwind 提供了七个通用的倾斜功能。您可以通过自定义 Tailwind 主题配置的 `skew` 部分来改变，添加或删除这些功能。
