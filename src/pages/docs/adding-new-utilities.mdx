---
title: 添加新的功能类
description: 使用你的自定义功能类来扩展 Tailwind。
---
...
确定扩展框架的最佳方法非常不易，因此这里有一些最佳实践，可以帮助你以最惯用的方式添加自己的功能类。
...
将自己的功能类添加到 Tailwind 的最简单的方式是直接添加到你的 CSS 中。
...
如果你想根据你的 `tailwind.config.js` 定义的断点创建功能类的变体，请将你的功能类放在  `@variants` 指令中，并把 `responsive` 添加到变体列表中。
...
如果你想生成你自己的功能类的 [dark mode variants](/docs/dark-mode)，首先确保在你的 `tailwind.config.js` 文件中 `darkMode` 被设置为 `media` 或者 `class`。
...
下一步，将你的功能类放在 `@variants` 指令中，并且把 `dark` 添加到变体列表中。
...
如果你想为你的功能类生成 [状态变体](/docs/hover-focus-and-other-states)，请将你的功能类放在 `@variants` 指令中，并列出你想启用的变体：
...
如果你想把你的自定义功能类作为一个库发布，或者使其更容易跨项目分享，这是一个不错的选择。
