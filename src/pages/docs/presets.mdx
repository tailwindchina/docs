...
默认情况下，你的 `tailwind.config.js` 文件中添加的任何配置将被智能的与 [默认配置](https://unpkg.com/browse/tailwindcss@^2/stubs/defaultConfig.stub.js) 合并，你自己的配置则作为一组覆盖与扩展。
...
`presets` 选项允许你指定一个_不同_的配置来用作你的基本配置，这使得打包一组跨项目复用的配置非常简单。
...
预设就是常规的 Tailwind 配置对象，与你的 `tailwind.config.js` 文件里添加的配置毫无二致。
...
**默认情况下，预设本身会扩展 Tailwind 的 [默认配置](https://unpkg.com/browse/tailwindcss@^2/stubs/defaultConfig.stub.js)，就像你自己的配置一样**。如果你想创建一个预设来完全代替默认配置，在预设本身中包含一个空的 `presets` 键。
...
项目特定的配置（那些在你的 `tailwind.config.js` 文件中找到的配置）与预设配置合并的方式与默认配置合并的方式相同。
...
`plugins` 数组在不同的配置中被合并，使预设可以注册插件，同时也允许你添加额外的项目级插件。
...
如果你把 `corePlugins` 配置为一个数组，它就会取代你配置的预设所提供的任何 `corePlugins` 配置。
...
`presets` 选项是一个数组，可以接受多个预设。如果你想把你的可重复使用的定制拆分成可以独立导入的组合块，这很有用。
...
在添加多个预设时，需要注意的是，如果它们以任何方式重叠，它们的解析方式和你自己的自定义与预设的解析方式是一样的，最后的配置获胜。
...
如果你想完全禁用默认配置并且不使用任何基础配置，请将 `presets` 设置为一个空数组。
...
如果你想让你的预设提供一个完整的设计系统，而不是扩展 Tailwind 的默认值，你也可以在预设中这样做。
