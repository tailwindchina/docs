...
首先使用 `transform` 功能启用转换，然后使用 `rotate-{angle}` 功能指定旋转角度来旋转一个元素。
...
要在特定的断点处控制元素的旋转，请在任何现有的旋转功能中添加 `{screen}:` 前缀。例如，使用 `md:rotate-90` 来应用 `rotate-90` 功能在中等尺寸以上的屏幕上。
...
默认情况下，Tailwind 提供了七个通用的旋转功能。您可以通过编辑您的 Tailwind 配置的 `theme.rotate` 部分来改变，添加或删除这些功能。
