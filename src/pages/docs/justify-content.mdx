...
description: "用于控制 flex 和 grid 项目如何沿着容器的主轴定位的工具。"
---
...
要在特定的断点处对 flex 项目应用 justify 工具，请在任何现有的工具类前添加 `{screen}:` 前缀。例如，使用 `md:justify-between` 来仅在中等尺寸以上的屏幕应用 `justify-between` 工具。
