...
我们在内部将其用于 `row-span-{n}` 功能。请注意，因为这直接配置了 `grid-row` 速记属性，所以我们直接在值名中包含了 `span` 这个词，而_不是_自动加入到类名中。这意味着你可以自由地添加你想做的任何事情--它们不只是 `span` 功能。
...
你可以通过修改 `tailwind.config.js` 文件中 `variants` 部分的 `gridRow`、`gridRowStart` 和 `gridRowEnd` 属性来控制为 grid-row 功能生成哪些变体。
