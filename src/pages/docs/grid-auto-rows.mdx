...
description: "用于控制隐式创建的网格行的大小的工具。"
---
...
使用 `auto-rows-{size}` 工具来控制隐式创建的网格行的大小。
...
要在特定的断点处控制 grid-auto-rows 属性，可以在任何现有的 grid-auto-rows 工具前添加 `{screen}:` 前缀。例如，使用 `md:auto-rows-min` 来仅在中等尺寸及以上的屏幕应用 `auto-rows-min` 功能。
...
默认情况下，Tailwind 包括四个通用的 grid-auto-rows 工具。您可以在您的 `tailwind.config.js` 文件的 `theme.gridAutoRows` 部分定制这些工具。
