...
你的 `tailwind.config.js` 文件的 `theme.screens` 部分定义你项目的断点。键是您的屏幕名称（用于 Tailwind 生成的响应工具类变体的前缀，如`md:text-center`），值是 `min-width`，该断点应该开始的地方。
...
你可以自由地拥有你想要的多的屏幕，以任何你喜欢的方式为他们命名。
...
这些屏幕名称将反映在你的功能类中，所以你的 `text-center` 功能类现在看起来像这样：
...
如果你想使用最大宽度断点而不是最小宽度断点，你可以指定你的屏幕为具有 `max` 键的对象。
...
例如，如果有必要，你甚至可以同时用 `min-width` 和 `max-width` 定义创建断点。
...
例如，假设你有一个侧边栏，并希望你的断点是基于内容区域的宽度，而不是整个视口。您可以模拟这种情况，当侧边栏变得可见并缩小内容区域时，您的一个断点会回到一个较小的断点。
...
如果你需要为断点提供一个完全自定义的媒体查询，你可以使用一个带有 `raw` 键的对象来实现。
...
如果你需要专门为打印应用不同的风格，`raw` 选项可能特别有用。

<!-- All you need to do is add a `print` screen under `theme.extend.screens`: -->
你需要做的就是在 `theme.extend.screens` 下添加一个 `print` 屏幕：
...
然后，你可以使用像 `print:text-black` 这样的类来指定只有当有人试图打印你正在处理的页面时才会应用的样式：
