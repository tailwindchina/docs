...
使用 `m{t|r|b|l}-{size}` 工具控制元素一侧的外边距。
...
要在特定的断点处控制元素的外边距，请在任何现有的 margin 工具前添加 `{screen}:` 前缀。例如，将 `md:my-8` 类添加到一个元素中，就可以在中等及以上屏幕尺寸的情况下应用 `my-8` 功能。
...
默认情况下，Tailwind 为每个边和轴提供了 20 个固定的外边距工具。
...
如果你想一次性自定义 padding、margin、width 和 height 的值，请使用你的 `tailwind.config.js` 文件中的 `theme.spacing` 部分。
...
如果只想自定义外边距值，请使用你的 `tailwind.config.js` 文件中的 `theme.margin` 部分。
...
如果你想添加额外的负页边距类（采取`-m{side?}-{size}`的形式），在配置文件中的键前加上破折号。
...
Tailwind 是足够聪明的，当它看到前面的破折号时，会生成像 `-mx-72` 这样的类，而不是像你可能期望的 `mx--72`。
