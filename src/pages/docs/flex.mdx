...
description: "用于控制 flex 项目放大和收缩的工具。"
features:
  responsive: true
  customizable: true
  hover: false
...
使用 `flex-initial` 允许 flex 项目在考虑到其初始尺寸的情况下收缩但不放大：
...
使用 `flex-1` 允许 flex 项目根据需要放大和收缩，忽略其初始尺寸。
...
使用 `flex-auto` 允许一个 flex 项目在考虑到其初始大小的情况下放大和收缩：
...
使用 `flex-none` 来阻止一个 flex 项目的放大和收缩：
...
要控制 flex 项目在特定断点处的放大和收缩方式，可在任何现有的功能类产添加 `{screen}:` 前缀。例如，使用 `md:flex-1` 仅在中等尺寸及以上的屏幕应用 `flex-1` 功能。
...
默认情况下，Tailwind 提供了四个 `flex` 工具。您可以通过编辑您的 Tailwind 配置的 `theme.flex` 部分来改变、添加或删除这些工具。
