...
现在，只要用户的操作系统开启了深色模式，`dark:{class}` 类离将优先于无前缀的类。`media` 策略在底层使用 `prefers-color-scheme` 媒体功能，但是，如果您想支持手动切换深色模式，您也可以 [使用 'class' 策略](#toggling-dark-mode-manually) 进行更多控制。
