...
但是随着项目的成长，你会不可避免的发现自己重复使用通用的功能类组合在许多不同的地方创建相同的组件。这对于类似按钮、表单元素、徽章这样的小组件最为明显。
...
定义一个 UI 组件的所有信息可以完全存在于 CSS 中，这非常罕见。与此同时，你也需要使用一些重要的相对应的 HTML 结构。
...
通过为模板创建一个单一源，你可以继续使用功能类，而不会因为在多个地方复制相同的类而造成任何的维护负担。
...
为了避免意外的特定性问题，我们建议你使用 `@layer components { ... }` 指令包装你的自定义组件样式，以告诉 Tailwind 这些样式属于哪一层。
